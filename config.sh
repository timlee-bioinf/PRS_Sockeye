...
# see submit.local.sh.example). Do NOT hardcode personal paths / allocation
# here - keep those in submit.local.sh.
set -euo pipefail
...
# --- inputs (read-only; can live anywhere, e.g. shared lab storage) ---
...
# Setup output cache (see scripts/setup_ancestry_reference.sh) - must be
# writable. Not tied to RUN_ID - reused by every pipeline run. The
# PCA/classifier part is shared by all traits; the per-ancestry normalization
# models live under traits/<TRAIT>/.
ANCESTRY_REF_CACHE="${ANCESTRY_REF_CACHE:-$PROJECT_ROOT/ancestry_ref_cache}"
ANCESTRY_NORM_DIR="${ANCESTRY_NORM_DIR:-$ANCESTRY_REF_CACHE/traits/$TRAIT}"
